...
WARNING!
This file is generated automatically! To make changes, you need to edit the Python script.
Date of generation: {datetime.now().strftime("%d-%m-%Y %H:%M:%S")}
Feature count: {len(FEATURE_IDS)}
*/


USE {{{{ params.yt_cluster }}}};

PRAGMA yt.StaticPool = "{{{{ params.yt_pool }}}}";
PRAGMA yt.DefaultOperationWeight = "{{{{ params.operation_weight }}}}";
...
$start_date = CAST(CurrentUtcDate() - DateTime::IntervalFromDays({{{{ params.start_days_offset }}}}) as String);
$end_date = CAST(CurrentUtcDate() - DateTime::IntervalFromDays({{{{ params.end_days_offset }}}}) as String);
"""
    ).strip()
...
{features_for_select_block}
    ...
    WHERE
        ...
        AND navScreen IN String::SplitToList("{{{{ params.nav_screens }}}}", ",")
        AND platform IN String::SplitToList("{{{{ params.platforms }}}}", ",")
    ;
    """
    ...
    for idx in FEATURE_IDS:
        _for_union.append(
            f"\tSELECT date, platform, navScreen, formulaPath, 'feature_{idx}' AS feature_name, feature_value_{idx} AS feature_value "
...
"""
    ...
    _query += f"""
INSERT INTO `{{{{ params.path_to_features_with_statistics_table }}}}` WITH TRUNCATE
SELECT
    date,
    platform,
    navScreen,
...
if __name__ == "__main__":
    ...
    PATH_TO_POOL_CACHE_YT = "//home/hc/ucp/vk_video/pool_caches/1d/"
